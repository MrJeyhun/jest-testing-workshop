...
test('deleteUser will 403 if not requested by user', async () => {
  const {req, res} = setup()
  const testUser = await db.insertUser(generate.userData())
  req.params = {id: testUser.id}
...
test('deleteUser will 404 if the user is not found', async () => {
  const {req, res} = setup()
...
test('deleteUser will delete user if the user properly requested', async () => {
  const {req, res} = setup()
  const testUser = await db.insertUser(generate.userData())
  req.params = {id: testUser.id}
  req.user = {id: testUser.id}
